...
- Handle provider-specific response formats that deviate from OpenAI spec
...
- Model list endpoint — let user pick model from UI instead of editing JSON

## Simplified setup / zero-friction config
- API key input in onboarding UI — write to `api_keys.json` directly, no manual file edit

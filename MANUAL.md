...
| Fit-check, tracker, notes, CSV export | No — country-neutral |
...
**⬇ Export CSV** in the tracker header downloads `job-applications.csv` — every job you marked **Applied** or **Interested**, in one spreadsheet file. Nothing is sent anywhere; the file is generated by your own instance and lands in your downloads folder.

This is the handover path out of the tool. Everything the app knows that *you* created — which jobs you applied to, when, what came back, your notes and ratings — lives only in `data/jobs_v2.db`. The export gets it into a format any spreadsheet, and any other tool, can read.

Included, one row per job: user status (applied/interested), application status, position, company, location, applied date, last reaction, reaction date, notes, rating, fit score, fit label, application URL, detail URL, and the internal job ID.

Not included: scraped listings you never touched, deleted jobs, and the long AI text (fit summary, fit reasoning, full posting text). Scraped listings are reproducible by scraping again; the long text would make the spreadsheet unusable.

Notes:

- Rows are ordered applied first, newest application on top; interested jobs follow.
- Rating and fit score are left **blank** when never set, rather than shown as `0`.
- Multi-line notes stay inside a single cell. Quotes and commas in your text survive intact.
- The file is UTF-8 with a byte-order mark, so Excel renders `Zürich` correctly on a double-click. If your tool asks for an encoding anyway, choose UTF-8.
- The export is never cached — each click gives you the current state of the database.

Import is not implemented yet; this is a one-way door out.

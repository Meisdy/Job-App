...
| `src/main.cpp` | `/api/jobs/import-text` uses `std::localtime` (not thread-safe) inside httplib request handlers, which run in a thread pool. Use `localtime_r` or `std::chrono`. | Data race / UB |
